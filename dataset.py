...
class RallyGroupedBatchSampler(Sampler):
    """Batch sampler that groups samples by rally to maximize cache hits.

    Shuffles rally order each epoch and shuffles samples within each rally,
    but keeps each batch drawn from as few rallies as possible. This means
    the per-rally NPZ cache (size 2) will almost always hit.
    """
    ...
    def __init__(self, dataset, batch_size, drop_last=False):
        ...
        # Build rally -> [sample indices] mapping from dataset id array
        # id shape: (N, seq_len, 2) where [:,0,0] is rally index
        rally_ids = dataset.data_dict['id'][:, 0, 0]
        self.rally_to_indices = {}
        for idx, rally_id in enumerate(rally_ids):
            rally_id = int(rally_id)
            if rally_id not in self.rally_to_indices:
                self.rally_to_indices[rally_id] = []
            self.rally_to_indices[rally_id].append(idx)

        self._len = None

    def __iter__(self):
        # Shuffle rally order
        rally_keys = list(self.rally_to_indices.keys())
        np.random.shuffle(rally_keys)

        # Build a stream of indices: rallies in random order, samples within each rally shuffled
        all_indices = []
        for rally_id in rally_keys:
            indices = self.rally_to_indices[rally_id].copy()
            np.random.shuffle(indices)
            all_indices.extend(indices)

        # Chunk into batches (rally order already shuffled above)
        # Don't shuffle batches — keeping rally-adjacent batches together
        # is critical for cache hits. Rally order is already random.
    ...
    def __len__(self):
        if self._len is None:
            total = sum(len(v) for v in self.rally_to_indices.values())
            if self.drop_last:
                self._len = total // self.batch_size
            else:
                self._len = (total + self.batch_size - 1) // self.batch_size
        return self._len
...
class Shuttlecock_Trajectory_Dataset(Dataset):
    ...
    def _load_precomputed_frames(self, frame_file_path):
        """Load precomputed frames for the rally containing this frame.
        ...
        rally_dir = os.path.dirname(frame_file_path)

        if rally_dir in self._pc_rally_cache:
            return self._pc_rally_cache[rally_dir]

        rel_path = os.path.relpath(rally_dir, self.root_dir)
        safe_name = rel_path.replace(os.sep, '_')

        # Prefer uncompressed .npy (faster to load than .npz)
        npy_file = os.path.join(self._pc_dir, f'{safe_name}.npy')
        npz_file = os.path.join(self._pc_dir, f'{safe_name}.npz')

        if os.path.exists(npy_file):
            frames = np.load(npy_file)
        elif os.path.exists(npz_file):
            frames = np.load(npz_file)['frames']
        else:
            self._pc_rally_cache[rally_dir] = None
            return None

        # LRU cache: keep last 5 rallies (~500MB-1GB, safe for 16GB machines)
        if len(self._pc_rally_cache) >= 5:
            oldest = next(iter(self._pc_rally_cache))
            del self._pc_rally_cache[oldest]

        self._pc_rally_cache[rally_dir] = frames
        return frames

    @staticmethod
    def _frame_num(frame_path):
        """Extract integer frame number from path like 'data/.../42.png' -> 42"""
        ...
        if self.frame_arr is not None:
            ...
            if self.frame_alpha > 0:
                ...
                if self.bg_mode == 'subtract':
                    ...
                    C = 3
                n_total = 2 * self.seq_len - 1  # 15 for seq_len=8

                # Pre-allocate arrays (avoids O(n^2) np.concatenate in loop)
                all_frames = np.empty((n_total, C, self.HEIGHT, self.WIDTH), dtype=np.float32)
                all_heatmaps = np.zeros((n_total, 1, self.HEIGHT, self.WIDTH), dtype=np.float32)
                all_coor = np.empty((n_total, 2), dtype=np.float32)
                all_vis = np.empty((n_total, 1), dtype=np.float32)

                def _load_frame(fi):
                    fn = self._frame_num(frame_file[fi])
                    if pc_frames is not None and fn < len(pc_frames):
                        return pc_frames[fn]
                    img = Image.open(frame_file[fi])
                    if self.bg_mode == 'subtract':
                        img = Image.fromarray(np.sum(np.absolute(img - median_img), 2).astype('uint8'))
                        return np.array(img.resize(size=(self.WIDTH, self.HEIGHT))).reshape(1, self.HEIGHT, self.WIDTH)
                    elif self.bg_mode == 'subtract_concat':
                        diff_img = Image.fromarray(np.sum(np.absolute(img - median_img), 2).astype('uint8'))
                        diff_img = np.array(diff_img.resize(size=(self.WIDTH, self.HEIGHT))).reshape(1, self.HEIGHT, self.WIDTH)
                        img = np.moveaxis(np.array(img.resize(size=(self.WIDTH, self.HEIGHT))), -1, 0)
                        return np.concatenate((img, diff_img), axis=0)
                    else:
                ...
                all_heatmaps[0] = prev_heatmap
                all_coor[0] = prev_coor
                all_vis[0] = prev_vis
                ...
                for i in range(1, self.seq_len):
                    ...
                    if vis[i] == 0:
                        ...
                        all_heatmaps[slot] = prev_heatmap
                        all_coor[slot] = prev_coor
                        all_vis[slot] = prev_vis
                        ...
                        cur_heatmap = self._get_heatmap(int(coor[i][0] / w_scaler), int(coor[i][1] / h_scaler))
                        all_heatmaps[slot] = prev_heatmap * lamb32 + cur_heatmap * (1.0 - lamb32)
                        all_coor[slot] = coor[i]
                        all_vis[slot] = vis[i]
                ...
                frames = all_frames[rand_id]
                heatmaps = all_heatmaps[rand_id, 0]  # drop the leading dim -> (seq_len, H, W)
                tmp_coor = all_coor[rand_id]
                tmp_vis = all_vis[rand_id]
                ...
                for i in range(self.seq_len):
                    fn = self._frame_num(frame_file[i])
                    if pc_frames is not None and fn < len(pc_frames):
                        img = pc_frames[fn]
                    else:
